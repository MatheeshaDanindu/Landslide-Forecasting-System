...
- **64x64 @ 10m patches, stride 64 (640m coverage).** Cut down from an original 256x256 (2.56km) sizing to make training tractable on available compute — roughly 16x fewer pixels per patch. Reliably covers the median (~63m) and p90 (~260m) polygon width with margin, but clips context for the largest landslides (p99 width ~917m, max ~3.5km); those still get a positive sample, just without full surrounding context. A speed tradeoff, not a scientific improvement — document as a limitation.
